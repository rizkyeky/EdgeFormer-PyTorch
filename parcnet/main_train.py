...
def main(opts, **kwargs):
    ...
    if system_type != 'mac':
        if num_gpus == 0:
            ...
            model = model.to(device=device)
            if is_master_node:
                logger.log('Using 1 GPU for training')
        else:
    ...
    if resume_loc is not None or auto_resume:
        model, optimizer, gradient_scalar, start_epoch, start_iteration, best_metric, model_ema, history = load_checkpoint(
            opts=opts,
            model=model,
            optimizer=optimizer,
            model_ema=model_ema,
...
def main_worker(**kwargs):
    ...
    system_type = getattr(opts, "eky.system")
    ...
    if system_type == 'mac':
        ...
        setattr(opts, "model.detection.pretrained", 'checkpoints/checkpoint_cls_last.pt')
    else:
